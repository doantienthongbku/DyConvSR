...
class DynamicConv(nn.Module):
    def __init__(self,in_planes,out_planes,kernel_size,stride=1,padding=0,dilation=1,grounps=1,bias=True,K=4,temperature=30,ratio=4,init_weight=True):
        super().__init__()
        ...
        self.attention=ControlModule(in_planes=in_planes,ratio=ratio,K=K,temperature=temperature,init_weight=init_weight)
...
if __name__ == '__main__':
    ...
    control_module = ControlModule(16, 4, K=3)
    y = control_module(x)
    print(y.shape)
    
    # test DynamicConvV2
    x = torch.randn(2, 16, 64, 64)
    dynamic_conv = DynamicConv(16, 32, 3, 1, padding=1, K=3)
    y = dynamic_conv(x)
    print(y.shape)
    
    # count parameters
    num_params = 0
    for p in dynamic_conv.parameters():
        num_params += p.numel()
    print('num_params:', num_params) # num_params: 109824